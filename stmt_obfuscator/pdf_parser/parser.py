...
class PDFParser:
    ...
    def validate_pdf_structure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not has_text:
                validation_results["warnings"].append("PDF may not contain extractable text")
            ...
            if form_fields:
                validation_results["info"].append(f"PDF contains {len(form_fields)} form fields")
            
            # Store validation results
            self.document_structure.validation_results = validation_results
            self.validation_errors = validation_results["errors"]
            
            logger.info(f"PDF validation completed: valid={validation_results['valid']}")
                ...
                if block["type"] == 0:  # Text block
                    ...
                    for line_idx, line in enumerate(block["lines"]):
                        ...
                        for span in line["spans"]:
                            ...
                            is_italic = bool(font_flags & 1)  # Check if italic flag is set
    ...
    def detect_tables(self) -> List[Table]:
        ...
        for page_num, page in enumerate(self.document):
            ...
            page_blocks = [block for block in self.text_blocks if block.page == page_num + 1]
            ...
            potential_tables = self._identify_potential_tables(rows, page_width, page_height)
            ...
            for table_idx, potential_table in enumerate(potential_tables):
                ...
                for row_idx, row in enumerate(rows_data):
                    for col_idx, cell_block_indices in enumerate(row):
                        ...
                        cell_text = " ".join([page_blocks[idx].text for idx in cell_block_indices])
                        
                        # Get bounding box for the cell
                        cell_blocks = [page_blocks[idx] for idx in cell_block_indices]
                        cell_bbox = self._combine_bboxes([block.bbox for block in cell_blocks])
    ...
    def _group_blocks_by_rows(self, blocks: List[TextBlock]) -> List[List[int]]:
        ...
        sorted_blocks = sorted(enumerate(blocks), key=lambda x: (x[1].bbox[1], x[1].bbox[0]))
        ...
        for i, row in enumerate(rows):
            ...
            if self._rows_have_similar_alignment(prev_row, row):
                ...
                if len(current_table_rows) >= 3:
                    ...
                    all_blocks = [self.text_blocks[idx] for row in current_table_rows for idx in row]
        ...
        for row in table_rows:
            ...
            for block_idx in row:
                ...
                for col_idx in range(len(column_boundaries) - 1):
                    if column_boundaries[col_idx] <= block_x < column_boundaries[col_idx + 1]:
    ...
    def _combine_bboxes(self, bboxes: List[Tuple[float, float, float, float]]) -> Tuple[float, float, float, float]:
        """
    ...
    def _determine_table_section(self, table_bbox: Tuple[float, float, float, float], page_num: int) -> str:
        """
        ...
        potential_headers = [
            ...
            if block.page == page_num + 1 and block.bbox[3] < table_y_top and block.bbox[3] > table_y_top - 50
        ]
    ...
    def _analyze_layout(self) -> Dict[str, Any]:
        ...
        for page_num, page in enumerate(self.document):
            ...
            if page_blocks:
                ...
                bottom_margin = page_height - max(block.bbox[3] for block in page_blocks)
                ...
                headers = [
                    ...
                    if block.bbox[1] < top_margin + 50 and (block.is_bold or block.size > 10)
                ]
                ...
                density_map = self._calculate_text_density(page_blocks, page_width, page_height)
                ...
                if density_grid[y, x] > 0.5 and not visited[y, x]:
                    ...
                    if len(region) > 4:  # Minimum size for a region
                        ...
                        high_density_regions.append({
                            "bbox": region_bbox,
                            "size": len(region),
                            "avg_density": sum(density_grid[y, x] for x, y in region) / len(region)
                        })
    ...
    def _flood_fill(
        ...
        if (
            x < 0 or y < 0 or 
            x >= grid.shape[1] or y >= grid.shape[0] or 
    ...
    def _identify_columns(self, blocks: List[TextBlock], page_width: float) -> List[Dict[str, Any]]:
        """
        ...
        x_ends = [block.bbox[2] for block in blocks]
        ...
        for i in range(len(column_boundaries) - 1):
            ...
            if column_blocks:
                columns.append({
                    "index": i,
                    "bbox": (left, 0, right, max(block.bbox[3] for block in column_blocks)),
    ...
    def get_text_for_pii_detection(self) -> Dict[str, Any]:
        ...
        all_blocks = sorted(self.text_blocks, key=lambda b: (b.page, b.bbox[1], b.bbox[0]))
    ...
    def chunk_document_for_pii_detection(self, max_chunk_size: int = 4000) -> List[Dict[str, Any]]:
        """
        ...
        for section_name, section_data in document_text["sections"].items():
            ...
            if len(section_text) <= max_chunk_size:
                ...
                section_blocks = [
                    block for block in document_text["text_blocks"]
                ...
                chunks.append({
                    ...
                    "tables": [
                        table for table in document_text["tables"]
                ...
                section_blocks.sort(key=lambda b: (b["page"], b["bbox"][1], b["bbox"][0]))
                ...
                for block in section_blocks:
                    ...
                    if len(current_chunk["text"]) + len(block["text"]) > max_chunk_size and current_chunk["text"]:
                    ...
                    if table["section"] == section_name:
                        ...
                        for chunk in chunks:
                            if chunk["section"] == section_name:
                                chunk_pages = set(block["page"] for block in chunk["blocks"])
